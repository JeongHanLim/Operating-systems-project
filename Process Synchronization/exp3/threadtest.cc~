...
{
	//fill your code
	int a=value;			//copy and paste from inc & dec respectively
	a++;
	currentThread->Yield();	  //put here to induce race condition. or can put in Dec_v1 oso
	value=a;	//cannot put yield after this line bcos only left printf wun cause race condition
...
void TestValueOne()
{
	...
	Inc1->Fork(Inc_v1, 0, 1);		//define thread -> fork it -> join all threads
...
void TestValueMinusOne()
{
	...
	Inc1->Fork(Inc_v2, 0, 1);		//define thread -> fork it -> join all threads
...
//fill your code
Semaphore *s = new Semaphore("idk", 1);
...
void TestConsistency()
{
	...
	Inc1->Fork(Inc_Consistent, 0, 1);		//define thread -> fork it -> join all threads

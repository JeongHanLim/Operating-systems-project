...
Machine::tableprint()
{
	DEBUG('m', "%i", currentThread->pid);
	for(int j=0; j<NumPhysPages; j++)
	{
		DEBUG('m', " [%i,%i,%i,%i]", memoryTable[j].pid, memoryTable[j].vPage, memoryTable[j].lastUsed, memoryTable[j].valid);
	}
	for(int j=0; j<TLBSize; j++)
	{
		DEBUG('m', "[%i,%i,%i]", machine->tlb[j].virtualPage, machine->tlb[j].physicalPage, machine->tlb[j].valid);
	}
	printf("\n");
}
